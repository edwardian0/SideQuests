...
void run (std::vector<std::string>& args)
{
  ...
  ShotgunSequencer sequencer(fragments);
  sequencer.init (fragments);

  std::cerr << "initial sequence has size " << sequencer.sequence().size() << "\n";

  while (sequencer.iterate());

  sequencer.check_remaining_fragments();

  std::cerr << "final sequence has length " << sequencer.sequence().size() << "\n";

  write_sequence (args[2], sequencer.sequence());
}
